...
function LenderDashboard({ data }) {
  ...
  return (
    <div>
      ...
      {/* Premium stat cards with left accent borders */}
      <div className="grid grid-cols-3 gap-3 mb-6">
        {[
          {label:"Private Money", val:privPrin, accent:"border-l-sky-500", num:"text-sky-700 dark:text-sky-300"},
          {label:"Hard Money",    val:hardPrin, accent:"border-l-amber-500",num:"text-amber-700 dark:text-amber-300"},
          {label:"Total Balance", val:totalBal, accent:"border-l-blue-500", num:"text-blue-700 dark:text-blue-300"},
        ].map(({label,val,accent,num})=>(
          <div key={label} className={`bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 border-l-4 ${accent} rounded-2xl p-4`}>
            <div className="text-[10px] font-semibold text-slate-400 dark:text-zinc-500 uppercase tracking-widest mb-2">{label}</div>
            <div className={`text-2xl font-bold tabular-nums ${num}`}>{$$(val)}</div>
          </div>
        ))}
      </div>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        <div className="bg-slate-900 dark:bg-zinc-800 rounded-2xl p-4 text-white">
          <div className="text-[9px] font-semibold text-slate-400 uppercase tracking-widest mb-2">Under Mgmt</div>
          <div className="text-2xl font-bold tabular-nums">{$$(haveNow)}</div>
          <div className="text-[10px] text-slate-500 mt-1">deployed + unassigned</div>
        </div>
        <div className="bg-blue-600 rounded-2xl p-4 text-white">
          <div className="text-[9px] font-semibold text-blue-200 uppercase tracking-widest mb-2">On Deals</div>
          <div className="text-2xl font-bold tabular-nums">{$$(totalDeployed)}</div>
        ...
        <div className="bg-violet-600 rounded-2xl p-4 text-white">
          ...
          <div className="text-2xl font-bold tabular-nums">{$$(unassignedTotal)}</div>
